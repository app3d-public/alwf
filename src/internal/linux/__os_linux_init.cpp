...
namespace alwf
{
    ...
    static void app_scheme_request_cb(WebKitURISchemeRequest *request, gpointer)
    {
        ...
        {
            GInputStream *stream =
                g_memory_input_stream_new_from_data(file.content.c_str(), file.content.size(), nullptr);
    ...
    static WebKitWebView *on_create_web_view(WebKitWebView *webview, WebKitNavigationAction *nav,
                                             gpointer /*user_data*/)
    {
        ...
        if (jsc_value_is_string(value))
        {
            ...
            if (!doc.HasParseError() && doc.HasMember("handler"))
            {
                ...
                if (it != env.handlers.end())
                    ...
                    printf("Failed to find handler: %s\n", handler.c_str());
            }
            g_free(json_str);
        }
    }
    ...
    void send_json_to_frontend(const rapidjson::Value &json)
    {
        ...
        json.Accept(writer);
        acul::string script = acul::format("window.__alwf_receive(%s);", buffer.GetString());
